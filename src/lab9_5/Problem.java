...
public class Problem {
	...
	public static List<String> elementsInBoth(List<String> list1, List<String> list2) {
		//implement
		List<String> list = Optional.ofNullable(list1)
				.orElseGet(List::of)
				.stream()
				.filter(item -> Optional.ofNullable(list2).orElseGet(List::of).contains(item))
	...
	public static List<String> getZipsOfSpecialCustomers(List<Customer> list) {
		...
		return Optional.ofNullable(list)
				.orElseGet(List::of)
				.stream()
				.filter(item -> item.getCity().length() >= 6 && !item.getCity().contains("e"))
				.map(Customer::getZip)
				.distinct()
